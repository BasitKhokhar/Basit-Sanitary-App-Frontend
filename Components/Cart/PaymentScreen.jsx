...
const PaymentScreen = () => {
  ...
  const openApp = async (appUrl) => {
    try {
      ...
      if (supported) {
        ...
        Alert.alert(
          "App Not Found",
          "Please install the app or make payment manually from your JazzCash/EasyPaisa app and upload receipt in confirm order form."
        );
      }
    } catch (error) {
      Alert.alert("Error", "Unable to open the app.");
  ...
  const openJazzCash = () => openApp("jazzcash://");
  const openEasyPaisa = () => openApp("easypaisa://");

  return (
    <View style={[styles.container, { backgroundColor: colors.bodybackground }]}>
      <Text style={[styles.header, { color: colors.text }]}>Choose Payment Method</Text>

      <TouchableOpacity 
        style={[styles.paymentButton, { backgroundColor: colors.primary }]}
        onPress={() =>
          navigation.navigate("StripePayment", { advance_payment, user_email })
        }
      >
      ...
      <TouchableOpacity 
        style={[styles.backButton, { backgroundColor: colors.headerbg }]} 
